    ...
    for step in range(total_step):
        ...
        with conditional_autocast(not args.no_amp):
            ...
            for n, loss_function in enumerate(losses):
                ...
                curr_loss = loss_function(y_true=inputs[1], y_pred=ret_dict['moved_vol'], msk_true=y_true[n],
    ...
    with torch.no_grad():
        ...
        for val_step in range(math.ceil(len(val_subject) / args.batch_size)):
            ...
            with conditional_autocast(not args.no_amp):
                ...
                loss_list_val = []
                for n, loss_function in enumerate(losses):
                    curr_loss = loss_function(y_true=inputs[1],y_pred=ret_dict['moved_vol'],msk_true=y_true[n], msk_pred=y_pred[n], ignore_label=label_ignore) * weights[n]
